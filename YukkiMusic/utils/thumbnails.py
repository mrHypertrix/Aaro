...
async def gen_thumb(videoid, user_id, theme):
    ...
    try:
        ...
        try:
            text_w, text_h = draw.textsize(f"ARCH MUSIC IS PLAYING", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"ARCH MUSIC IS PLAYING",
                fill="red",
                font=font,
            )
...
async def gen_qthumb(videoid, user_id, theme):
    ...
    try:
        ...
        para = textwrap.wrap(title, width=32)
        try:
            text_w, text_h = draw.textsize(f"Arch ADDED THIS SONG TO QUEUE", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"Arch ADDED THIS SONG TO QUEUE",
                fill="red",
                font=font,
            )
            text_w, text_h = draw.textsize(
